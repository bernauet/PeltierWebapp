...
const i18n = {
    en: {
        ...
        menu_plant_title: "Plant Identification",
        menu_plant_desc: "Executes automatic voltage steps and records them in a CSV file for system identification",
        btn_back: "Back",
        manual_title: "Manual Voltage Control",
        btn_start_log: "Start CSV",
        btn_stop_log: "Stop CSV",
        btn_apply_voltage: "Apply Voltage",
        label_applied: "Applied (u)",
        ref_title: "Reference Testing",
        ref_select: "Select Controller...",
        ref_opt_mpc_imp: "MPC (Implicit Model Predictive)",
        ref_opt_mpc_exp: "MPC (Explicit Model Predictive)",
        ref_opt_hyst_pur: "ON-OFF (Pur)",
        ref_opt_hyst_bang: "Bang-Bang (Aggressive)",
        ref_opt_hyst_hybrid: "Bang-Bang to PI-D (Hybrid)",
        ref_opt_fuzzy_pi: "Fuzzy PI Rapid Controller",
        ref_opt_fuzzy_mpc: "Fuzzy MPC Controller",
        btn_start_control: "Start Control",
        plant_title: "Plant Identification",
        status_log_inactive: "Log Inactive",
        status_log_active: "Log Active",
        plant_desc: "Executes automatic voltage steps and records them in a CSV file for system identification",
        ...
        csv_rules_title: "CSV Rules:",
        csv_rule_1: "2 columns (no headers): Time(s), Target(ºC).",
        csv_rule_2: "First row must start at time 0.",
        csv_rule_3: "Constant time step (Ts) between points.",
        csv_rule_4: "Minimum time step: Ts >= 0.7s."
    },
    ca: {
        ...
        btn_folder: "Assignar Carpeta CSV",
        ...
        menu_ref_desc: "Avalua controladors en llaç tancat (PID, MPC, Fuzzy...) amb consignes de temperatura precises",
        menu_plant_title: "Identificació de planta",
        menu_plant_desc: "Executa esglaons de voltatge automàtics i realitza la seva gravació en un fitxer CSV per a la identificació del sistema",
        btn_back: "Tornar",
        manual_title: "Control Manual de Voltatge",
        btn_start_log: "Iniciar CSV",
        btn_stop_log: "Aturar CSV",
        btn_apply_voltage: "Aplicar Voltatge",
        label_applied: "Aplicat (u)",
        ref_title: "Test de Referència",
        ref_select: "Selecciona Controlador...",
        ref_opt_mpc_imp: "MPC (Predictiu per Model Implícit)",
        ref_opt_mpc_exp: "MPC (Predictiu per Model Explícit)",
        ref_opt_hyst_pur: "ON-OFF (Pur)",
        ref_opt_hyst_bang: "Bang-Bang (Agressiu)",
        ref_opt_hyst_hybrid: "Bang-Bang a PI-D (Híbrid)",
        ref_opt_fuzzy_pi: "Controlador Fuzzy PI Ràpid",
        ref_opt_fuzzy_mpc: "Controlador Fuzzy MPC",
        btn_start_control: "Iniciar Control",
        plant_title: "Identificació de planta",
        status_log_inactive: "Registre Inactiu",
        status_log_active: "Registre Actiu",
        plant_desc: "Executa esglaons de voltatge automàtics i realitza la seva gravació en un fitxer CSV per a la identificació del sistema",
        ...
        csv_rules_title: "Normes CSV:",
        csv_rule_1: "2 columnes (sense capçalera): Temps(s), Objectiu(ºC).",
        csv_rule_2: "El primer valor ha de ser a temps 0.",
        csv_rule_3: "Increment de temps (Ts) sempre constant.",
        csv_rule_4: "Increment mínim permès: Ts >= 0.7s."
    },
    es: {
        ...
        btn_folder: "Asignar Carpeta CSV",
        status_no_folder: "Ninguna carpeta seleccionada",
        menu_title: "Selecciona el Modo Operativo",
        ...
        menu_plant_title: "Identificación de planta",
        menu_plant_desc: "Ejecuta escalones de voltaje automáticos y realiza su grabación en un archivo CSV para la identificación del sistema",
        btn_back: "Volver",
        manual_title: "Control Manual de Voltaje",
        btn_start_log: "Iniciar CSV",
        btn_stop_log: "Detener CSV",
        btn_apply_voltage: "Aplicar Voltaje",
        label_applied: "Aplicado (u)",
        ref_title: "Prueba de Referencia",
        ref_select: "Selecciona Controlador...",
        ref_opt_mpc_imp: "MPC (Predictivo por Modelo Implícito)",
        ref_opt_mpc_exp: "MPC (Predictivo por Modelo Explícito)",
        ref_opt_hyst_pur: "ON-OFF (Puro)",
        ref_opt_hyst_bang: "Bang-Bang (Agresivo)",
        ref_opt_hyst_hybrid: "Bang-Bang a PI-D (Híbrido)",
        ref_opt_fuzzy_pi: "Controlador Fuzzy PI Rápido",
        ref_opt_fuzzy_mpc: "Controlador Fuzzy MPC",
        btn_start_control: "Iniciar Control",
        plant_title: "Identificación de planta",
        status_log_inactive: "Registro Inactivo",
        status_log_active: "Registro Activo",
        plant_desc: "Ejecuta escalones de voltaje automáticos y realiza su grabación en un archivo CSV para la identificación del sistema",
        ...
        csv_rules_title: "Normas CSV:",
        csv_rule_1: "2 columnas (sin cabecera): Tiempo(s), Objetivo(ºC).",
        csv_rule_2: "El primer valor debe ser a tiempo 0.",
        csv_rule_3: "Incremento de tiempo (Ts) siempre constante.",
        csv_rule_4: "Incremento mínimo permitido: Ts >= 0.7s."
    }
};
    ...
    state.socket.onmessage = (event) => {
        ...
        lines.forEach(data => {
            ...
            if (parts.length >= 4 && !isNaN(parts[0])) {
                ...
                if (parts.length >= 7) {
                    // Closed Loop
                    // Format: Time, ref, inTemp, outTemp, u_o, u, e, comp_time_ms
                    const time = parts[0];
...
els.btnRefSet.addEventListener('click', async () => {
    ...
    let ref = parseFloat(els.refTargetInput.value);
    ...
    else if (core === 'MPC_IMP') ctrlNum = 6;
    else if (core === 'MPC_EXP') ctrlNum = 7;
    else if (core === 'FUZZY_PI') ctrlNum = 11;
    else if (core === 'FUZZY_MPC') ctrlNum = 12;
    else if (core === 'HYST_PUR') ctrlNum = 8;
    else if (core === 'HYST_BANG') ctrlNum = 9;
    else if (core === 'HYST_HYBRID') ctrlNum = 10;
    ...
    if (ctrlNum) {
        ...
        if (seqState.mode === 'constant') {
            if (ref < 0) {
                ref = 0;
                els.refTargetInput.value = 0;
                showToast("Reference clamped to minimum 0ºC limit.");
            } else if (ref > 100) {
                ref = 100;
                els.refTargetInput.value = 100;
                showToast("Reference clamped to maximum 100ºC limit.");
            }
        } else if (seqState.mode === 'sequence') {
            ...
            for (let i = 0; i < seqState.items.length; i++) {
                ...
                if (item.target === null || item.duration === null || isNaN(item.target) || isNaN(item.duration) || item.duration <= 0) {
                    showToast(`Item ${i+1} has invalid parameters.`);
                    return;
                }
                if (item.target < 0) {
                    item.target = 0;
                    showToast(`Item ${i+1} reference clamped to 0ºC.`);
                    renderSequenceList();
                } else if (item.target > 100) {
                    item.target = 100;
                    showToast(`Item ${i+1} reference clamped to 100ºC.`);
                    renderSequenceList();
                }
                if (item.type === 'ramp' && item.duration < 3) {
                    showToast(`Item ${i+1} (Ramp) must have at least 3s.`);
                    return;
                }
            }
